...
const walk = (source, callback) => {
  let x = 0;
  let y = 0;

  source.forEach(({ n, dir }) => {
    for (let i = 0; i < n; i++) {
      if (dir === 'R') {
        x++;
      }
...
let matrix = {};
walk(source[0], ({ x, y }) => (matrix[`${x},${y}`] = 1));
console.log(matrix);
let res = [];
walk(source[1], ({ x, y }) => {
  const coord = `${x},${y}`;
...
distance = res
  .filter(Boolean)
  .map(_ => {
    const [x, y] = _.split(',').map(_ => Math.abs(parseInt(_, 10)));
    return x + y;
  })
  .sort((a, b) => (a - b < 0 ? 1 : -1))
  .reverse()[0];

if (distance !== aim) throw new Error(`fail: ${distance} != ${aim}`);
console.log('works!');
